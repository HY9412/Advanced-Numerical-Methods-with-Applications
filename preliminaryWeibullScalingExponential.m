...
for j=1:length(gammaU)
    ...
    for i=1:N
        ...
        lambdaIS2 = (a*gamma(1+1/b))/val1IS;
...
%         L2 = exp(LogL2);
        YtsIS=exprnd(1/lambdaIS2);
        L2 = (1/lambdaIS2)*(lambda1/(lambda1-lambdaIS2))^nEr*gampdf(YtsIS,nEr,1/(lambda1-lambdaIS2));
...
%% The following is ConAK algorithm conditional on Erlang + IS + CV(density selection Exponential)
...
%         L2CV = exp(LogL2CV); 
        lambdaIS2CV = (a*gamma(1+1/b))/val1ISCV;
        YtsISCV=exprnd(1/lambdaIS2CV);
        L2CV = (1/lambdaIS2CV)*(lambda1/(lambda1-lambdaIS2CV))^nEr*gampdf(YtsISCV,nEr,1/(lambda1-lambdaIS2CV));
